#ifndef _MARKER_PATTERN_
#define _MARKER_PATTERN_
...
class Pattern
{
    ...
    /*
     * loads pattern of the marker
     */
    static int loadPattern(const char* filename, int msize, std::vector<cv::Mat>& library, int& patternCount);	
		/*
     ...
     */
		void draw3D(Mat& frame, const Mat& camMatrix, const Mat& distMatrix); 
		/*
     * augments image with 3D cubes. It;s too simple augmentation jsut for checking
     */
		void draw2D(Mat& frame, const Mat& camMatrix, const Mat& distMatrix);
		/*
