...
// marker_version = 0 => marker detection mode I - color
// marker_version = 1 => marker detection mode I - rectangle
// marker_version = 2 => marker detection mode II
int marker_version;
bool lane; 
...
int main(int argc, char **argv){
  ...
  while(ros::ok() && breakMainLoop == false){
    ...
    if(!boolLineFinder){
      ...
      }else if( marker_version == 0 ){
        foundMarker = marker.processMarkerTracking(inputImg, markerPosition);
